...
public class LuaDocParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // CLASS ID (EXTENDS class_name_ref)?
  ...
  /* ********************************************************** */
  // '@' (param_def | return_def | tag_def | class_def | field_def | type_def) comment_string?
  ...
  // comment_string?
  private static boolean doc_item_2(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "doc_item_2")) return false;
    comment_string(b, l + 1);
    return true;
  }

  /* ********************************************************** */
  // FIELD access_modifier ID type_set
  public static boolean field_def(PsiBuilder b, int l) {
    ...
    r = p && type_set(b, l + 1) && r;
    exit_section_(b, l, m, r, p, null);
    return r || p;
  }

  /* ********************************************************** */
  // TAG_PARAM OPTIONAL? param_name_ref type_set
  ...
  public static boolean type_list(PsiBuilder b, int l) {
    ...
    int c = current_position_(b);
    while (r) {
      if (!type_set(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "type_list", c)) break;
      c = current_position_(b);
    }
    exit_section_(b, l, m, r, false, null);
    return r;
  ...
  private static boolean type_set_2_0_0(PsiBuilder b, int l) {
    ...
    type_set_2_0_0_0(b, l + 1);
    return true;
  }

  // ','|'|'
  private static boolean type_set_2_0_0_0(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "type_set_2_0_0_0")) return false;
    boolean r;
    Marker m = enter_section_(b);
